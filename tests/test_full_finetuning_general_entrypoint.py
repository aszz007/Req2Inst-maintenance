"""Focused regression coverage for the general full-finetuning entry point."""
...
def test_general_full_finetuning_main_preserves_config_and_training_order(
    monkeypatch,
    capsys,
):
    events = []
    ...
    class FakeTrainer:
        output_dir = Path("checkpoints/full_finetuning/general_expert")
    ...
    assert "Data sources: text + image + FlowChart (mixed dataset)" in output
    assert "Sample coverage: about 85% of General (mixed dataset)" in output
    assert "Training completed successfully!" in output
    assert "checkpoints\\full_finetuning\\general_expert" in output
