"""Focused regression coverage for the FlowChart full-finetuning entry point."""
...
def test_flowchart_full_finetuning_main_preserves_config_and_training_order(
    monkeypatch,
    capsys,
):
    events = []
    ...
    class FakeTrainer:
        output_dir = Path("checkpoints/full_finetuning/uml_expert")
    ...
    assert "Sample coverage: about 70% of FlowChart" in output
    assert "Training completed successfully!" in output
    assert "checkpoints\\full_finetuning\\uml_expert" in output
